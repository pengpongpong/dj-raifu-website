...
const Navbar = ({ navData }: { navData: NavbarProps }) => {
    ...
    return (
        <nav className="mt-8 daisy_navbar flex-col justify-between items-center gap-4 font-text">
            {navData?.image ? <Image src={urlForImage(navData?.image).url()} className="w-[150px] md:w-[200px] lg:w-[250px] xl:w-[300px]" width={2560} height={1440} priority alt="Dj Raifu logo" /> : ""}
