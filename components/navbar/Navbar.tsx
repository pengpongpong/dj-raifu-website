...
const Navbar = ({ navData }: { navData: NavbarProps }) => {
    const socialStyles = {
        container: "mt-2 flex gap-12 md:gap-16 lg:gap-24 invisible opacity-0 animate-[showVisibility_2s_2.5s_ease-in-out_forwards] motion-reduce:visible motion-reduce:animate-none",
    ...
    return (
        <nav className="mt-8 daisy_navbar flex-col justify-between items-center gap-4 font-text">
            {navData?.image
                ? <Image src={urlForImage(navData?.image).url()} className="w-[150px] md:w-[200px] lg:w-[250px] xl:w-[300px] translate-y-[40vh] scale-150 animate-[slideUp_2s_1s_ease-in-out_forwards] z-10" width={2560} height={1440} priority alt="Dj Raifu logo" />
                : ""}
            <Socials styles={socialStyles} />
            <ul className="mt-2 gap-4 md:gap-6 md:mt-4 lg:gap-8 xl:gap-12 xl:mt-6 3xl:gap-16 tracking-wide md:tracking-wider lg:tracking-widest invisible opacity-0 animate-[showVisibility_2s_2s_ease-in-out_forwards] motion-reduce:visible motion-reduce:animate-none">
                <li>
